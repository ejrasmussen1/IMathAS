...
if (isset($_POST['submit']) && $_POST['submit']=="Sign Up") {
    ...
    if ($page_newaccounterror=='') {//no error
        ...
        if (strlen($enrollkey)>0 && count($keylist)>1) {
            ...
            $stm->execute(array(':userid'=>$newuserid, ':courseid'=>$cid, ':section'=>Sanitize::stripHtmlTags($_POST['ekey2']), ':gbcomment'=>$code, ':latepass'=>$deflatepass));
        } else {
    ...
    if (!isset($studentid) && !isset($teacherid) && !isset($tutorid)) {  //have account, not a student
        ...
        if (strlen($enrollkey)==0 || (isset($_REQUEST['ekey']) && in_array($_REQUEST['ekey'], $keylist))) {
            if (count($keylist)>1) {
                ...
                $stm->execute(array(':userid'=>$userid, ':courseid'=>$cid, ':section'=>Sanitize::stripHtmlTags($_REQUEST['ekey']), ':latepass'=>$deflatepass));
            } else {
