...
$end = $now;
$getStart = Sanitize::simpleString($_GET['start']);
$getDays = Sanitize::simpleString($_GET['days']);
$getEnd = Sanitize::simpleString($_GET['end']);

if (!empty($getStart)) {
    $parts = explode('-', $getStart);;
    if (count($parts) == 3) {
        $start = mktime(0, 0, 0, $parts[0], $parts[1], $parts[2]);
    }
} else if (!empty($getDays)) {
    $start = $now - 60 * 60 * 24 * intval($getDays);
}

if (!empty($getEnd)) {
    $parts = explode('-', $getEnd);
    if (count($parts) == 3) {
