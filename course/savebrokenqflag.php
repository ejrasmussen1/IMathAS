...
$stm->execute(array(':broken'=>$_GET['flag'], ':id'=>$get_qsetid));
if ($stm->rowCount()>0) {
	$ischanged = true;
	if ($_GET['flag']==1) {
		$now = time();
		//DB $msg = addslashes('Question '.$_GET['qsetid'].' marked broken by '.$userfullname);
		$msg = 'Question '.$get_qsetid.' marked broken by '.$userfullname;
		...
		if (isset($CFG['GEN']['sendquestionproblemsthroughcourse'])) {
			...
			$stm->execute(array(':id'=>$get_qsetid));
			...
			$stm->execute(array(':courseid'=>$CFG['GEN']['sendquestionproblemsthroughcourse'], ':title'=>'Question #'.$get_qsetid.' marked as broken',
				':message'=>"<p>This is an automated message.  $userfullname has marked question #".$get_qsetid." as broken. Hopefully they follow up with you about what they think is wrong with it.</p>",
